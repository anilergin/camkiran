...
class Tax_Meta_Class {
  ...
  public function check_field_time() {
    ...
    if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {
      ...
      // Enqueu JQuery UI, use proper version.
      $this->enqueue_jqueryui();
      wp_enqueue_script( 'at-timepicker', $plugin_path . '/libs/jquery-ui-timepicker-addon.min.js', array( 'tmc-jquery-ui' ),false,true );
    }
  }
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function enqueue_jqueryui(){
    ...
    $plugin_path = $this->SelfPath;
    wp_enqueue_style( 'tmc-jquery-ui-css',  $plugin_path . '/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.css' );
    wp_enqueue_script( 'tmc-jquery-ui', $plugin_path . '/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array( 'jquery' ) );
  }
