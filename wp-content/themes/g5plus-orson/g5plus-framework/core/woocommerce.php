...
if (class_exists('WooCommerce') && !class_exists('G5Plus_Woocommerce')) {
    class G5Plus_Woocommerce
    {
        ...
        function define_filter()
        {
            ...
            add_filter('woocommerce_product_related_posts_relate_by_tag', array($this, 'g5plus_woocommerce_product_related_posts_relate_by_tag'));

            add_filter('woocommerce_product_related_posts_relate_by_category', array($this, 'g5plus_woocommerce_product_related_posts_relate_by_category'));

            add_action('pre_get_posts', array($this, 'search_in_category'));


        }

        function search_in_category($query)
        {
            if (!is_admin()
                && is_search()
                && ($query->get('post_type') === 'product')) {
                $product_cate = isset($_GET['product_cate']) ? $_GET['product_cate'] : '';
                if ('' !== $product_cate) {
                    $tax_query = $query->get('tax_query',array());
                    $tax_query[] = array(
                        'taxonomy'         => 'product_cat',
                        'terms'            => array($product_cate),
                        'field' => 'slug',
                        'operator' => 'IN'
                    );
        ...
        public function g5plus_woocommerce_product_related_posts_relate_by_tag()
        {
            $related_product_condition = G5Plus_Global::get_option('related_product_condition');
            return $related_product_condition['tag'] == 1 ? true : false;
        }

        public function g5plus_woocommerce_product_related_posts_relate_by_category()
        {
            $related_product_condition = G5Plus_Global::get_option('related_product_condition');
