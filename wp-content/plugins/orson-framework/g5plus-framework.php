...
if (!class_exists('G5plus_FrameWork')){
	class G5plus_FrameWork{
		...
		public function enqueue_admin_resources(){
			add_thickbox();
			// select2
			wp_enqueue_style('select2',plugins_url(PLUGIN_G5PLUS_FRAMEWORK_NAME. '/assets/plugins/select2/css/select2'. $this->min_suffix .'.css'),array(),'4.0.3','all');
			wp_enqueue_script('select2',plugins_url(PLUGIN_G5PLUS_FRAMEWORK_NAME . '/assets/plugins/select2/js/select2.full'. $this->min_suffix .'.js'),array('jquery'),'4.0.3',true);
			// datetimepicker
